...
- 24 小时长稳运行、正式 Docker 压测报告、pgvector/Mem0 记忆迁移和外部告警通道。

面试时要明确：当前 M6 已具备本地可演示的“客服闭环 + 记忆 + 限流 + 观测 + 质量评估 + DeepSeek + FAQ/RAG + 管理接口 + 容器编排 + 压测入口 + 自动评测”能力，但还不是完整线上生产系统；正式压测报告、24 小时长稳验证、pgvector 记忆迁移和外部告警通道仍需后续环境验证。
