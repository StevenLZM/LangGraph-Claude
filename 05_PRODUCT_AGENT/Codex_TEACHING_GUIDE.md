...
│   ├── idempotency.py           # /chat request_id 幂等记录，SQLite/Postgres
│   ├── session_lock.py          # Redis 会话分布式锁，本地内存 fallback
│   ├── routers/
│   │   └── admin.py             # M6 管理接口
│   └── middleware/
...
├── messaging/
│   ├── events.py                # RocketMQ 事件 envelope
│   ├── outbox.py                # SQLite/Postgres outbox
│   └── publisher.py             # RocketMQ 发布器
├── infra/
...
浏览器 GET /
  -> api/ui.py 返回客服工作台
  -> 前端生成 request_id，fetch("/chat")
  -> api/main.py::chat()
  -> ChatRequest 校验 user_id / session_id / request_id / message
  -> ChatRequestStore.start_request 写入 processing 幂等记录
  -> 若 request_id 已成功，返回首次响应并标记 request_status=replayed
  -> 若 request_id 正在处理，短暂等待；仍未完成则返回 202 request_processing
  ...
  -> SessionStore 用 expected_version 保存本轮消息和质量评估 metadata
  -> record_chat_request 记录 Prometheus 兼容指标
  -> RocketMQPublisher 写 outbox 并发布业务事件
  -> ChatRequestStore.complete_success 保存首次响应快照
  -> SessionLockManager 释放会话锁
...
| 请求校验 | `api/schemas.py::ChatRequest` | 校验 `user_id`、`session_id`、`request_id`、`message` |
| 幂等记录 | `api/idempotency.py::ChatRequestStore` | 按 `(user_id, session_id, request_id)` 去重并保存首次结果 |
...
```text
校验 -> 幂等 -> 会话锁 -> 限流 -> 记忆/会话 -> token 预算 -> 图调用 -> 质量评估 -> 指标 -> 保存 -> outbox -> 响应
```
...
- `request_id`：客户端重试同一条消息时必须保持不变，用于服务端幂等去重。
...
- `request_status`：`processed` 表示首次处理，`replayed` 表示命中幂等结果，`degraded` 表示预算降级。
...
会话表现在带 `version` 字段。`load_session()` 返回当前 version，`save_session(..., expected_version=N)` 保存时会检查版本：

```text
第一次保存：expected_version=0，只允许插入新 session
后续保存：WHERE session_id=? AND version=?
写入成功：version + 1
写入失败：抛 SessionVersionConflict，API 返回 409
```
...
## 13. 幂等与并发控制：客户端重试同一条消息怎么办

用户网络抖动、浏览器超时、移动端自动重试、网关重放，都可能让同一条 `/chat` 消息到达服务端多次。如果没有幂等控制，系统会重复调用 LLM、重复保存会话、重复发布 RocketMQ 事件，甚至重复触发退款等高风险动作。

### 13.1 `request_id` 是客户端和服务端的契约

客户端每发送一条新消息生成一个 `request_id`。如果这条消息需要重试，必须复用同一个 `request_id`：
...
服务端按 `(user_id, session_id, request_id)` 作为幂等 key。为什么不只用 `request_id`？因为不同用户或不同会话可能生成相同 id，作用域必须包含业务隔离维度。

### 13.2 `ChatRequestStore` 的状态机

文件：`api/idempotency.py`
...
- 首次请求：插入 `processing`，继续进入客服主链路。
- 重复请求且已成功：读取 `response_json`，返回第一次响应，`request_status="replayed"`。
- 重复请求且仍在处理：最多短暂等待；如果仍未完成，返回 `202 request_processing`。
- 重复请求且首次失败：返回首次失败的 HTTP status 和 detail。
- 同一个 `request_id` 换了 `message`：返回 `409 idempotency_key_conflict`。

这里还会保存 `message_hash`。它不是为了加密消息，而是为了识别“客户端错误地把同一个 `request_id` 用在了不同消息上”。

### 13.3 会话锁保护同一 session 的顺序

幂等只能处理同一条消息重复请求，不能处理两条不同消息并发写同一个会话。所以 API 对新请求会先拿会话锁：

```text
lock:chat_session:{session_id}
Redis SET key token NX PX lease
release 用 Lua 校验 token 后删除
```
...
当前客服主路径已经要求真实 LLM 生成用户可见最终回答；规则层负责工具选择、安全边界和草稿上下文，`ResilientLLM` 负责 DeepSeek 主备模型、重试和熔断。
...
- UI 每次发送新消息生成新的 `request_id`；如果真实客户端做自动重试，应复用同一次发送的 `request_id`。
...
- 会话查询。
- 长期记忆保存、召回和删除。
- 每次请求必须携带唯一 `request_id`。

### 18.2 `tests/test_memory.py`

保护 M2 记忆系统：

- 短期窗口裁剪和摘要。
- SQLite 会话持久化。
- stale `expected_version` 会触发 `SessionVersionConflict`。
- 用户长期记忆保存、召回、删除。

### 18.3 `tests/test_storage_backends.py`
...
- `ChatRequestStore` 和 RocketMQ outbox 在 SQLite/Postgres 间按 `STORAGE_BACKEND` 切换。

### 18.4 `tests/test_chat_idempotency.py`

保护客户端重试语义：

- 同一个 `request_id` 重试同一条消息时返回首次结果。
- 重放响应设置 `request_status="replayed"`。
- 第二次请求不会再次调用 LLM。
...
- 内存 fallback 能串行化同一个 `session_id`。
- 第一个锁未释放时，同一会话第二个获取会返回 busy。
- 释放后可以重新获取锁。

### 18.6 `tests/test_checkpointer_factory.py`
...
### 18.7 `tests/test_rate_limiter.py`
...
### 18.8 `tests/test_llm_fallback.py`
...
### 18.9 `tests/test_observability.py`
...
### 18.10 `tests/test_deployment.py`
...
- 客户端重试同一条消息不会重复进入 LLM 主路径，而是从 `chat_requests.response_json` 重放首次结果。

### 19.2 当前已支持 LangGraph 原生 Checkpointer
...
这次改造补的是生产 API 必须面对的问题：客户端重试和多用户并发。当前已实现：

- `ChatRequest.request_id` 必填。
- `ChatRequestStore` 支持 SQLite/Postgres，本地测试用 SQLite，生产随 `STORAGE_BACKEND=postgres` 使用 Postgres。
- 重复完成请求返回首次结果，`request_status="replayed"`。
- 重复处理中请求会短暂等待，未完成返回 `202 request_processing`。
- 同一 `request_id` 换消息返回 `409 idempotency_key_conflict`。
- `SessionLockManager` 用 Redis 锁保护同一 `session_id` 的新请求。
- `SessionStore` 增加 version 乐观锁，防止静默覆盖。
- RocketMQ outbox 支持 Postgres，避免生产多实例写本地 SQLite。

限制也要讲清楚：当前 outbox 是请求内发送并记录状态，不是独立 relay worker；生产进一步增强应由 worker 扫描 pending/enqueue_failed 并重试。
...
### Q2：客户端重试同一条消息会不会重复调用 LLM？

回答：

> 不会。客户端重试必须复用同一个 `request_id`，服务端在进入 LLM 前先写 `chat_requests` 幂等表。首次请求处理完成后会保存完整响应；重复请求直接返回首次响应，并把 `request_status` 标记为 `replayed`。如果首次请求仍在处理中，服务端短暂等待，仍未完成就返回 `202 request_processing`，客户端可以再查状态。

### Q3：为什么还需要 Redis 会话锁，幂等表不够吗？

回答：

> 幂等表解决的是同一条消息重复提交；Redis 会话锁解决的是同一个 `session_id` 下两条不同消息并发处理。两条不同消息如果同时加载旧会话再分别保存，会导致顺序错乱或覆盖。Redis 锁让同一会话的新请求串行进入主链路，Session version 乐观锁再做数据库层兜底。

### Q4：为什么生产选 Postgres + Redis，而不是只用一个存储？

回答：

> Postgres 保存业务事实：会话、用户记忆、幂等记录、outbox 和 checkpoint，依赖唯一约束、事务更新和可审计查询。Redis 处理短期协调：限流计数和会话锁，依赖低延迟、TTL 和原子命令。SQLite 和内存 fallback 只用于本地演示和 pytest，不适合多实例生产。

### Q5：为什么要用 LangGraph，而不是 FastAPI 里一个函数写完？
...
FastAPI UI Page
   |
   | crypto.randomUUID() -> fetch POST /chat
   v
FastAPI API Layer
   |
   | validate + idempotency + session lock
   v
Redis / Postgres Consistency Layer
   |
   | Redis session lock + Postgres chat_requests
   v
FastAPI Runtime Controls
   |
   | rate limit + QPS + token budget
   v
Session / Memory Layer
   |
   | load session version + load user memories + trim context
   v
...
ChatResponse
   ...
   v
SessionStore / UserMemoryManager / Outbox / Metrics
   |
   | expected_version save + extract memory + RocketMQ events + expose /metrics
   v
Browser UI
...
curl -X POST http://127.0.0.1:8000/chat \
  ...
  -d '{"user_id":"user_001","session_id":"session_001","request_id":"demo_req_001","message":"Where is my order ORD123456？谢谢"}'
```
...
curl "http://127.0.0.1:8000/chat/requests/demo_req_001?user_id=user_001&session_id=session_001"
```
...
8. 并发一致性增强补齐了 `request_id` 幂等、Redis 会话锁、Session version 乐观锁和 Postgres outbox，客户端重试同一条消息不会重复调用 LLM。

面试收尾表达：

> 我在这个项目里关注的不是“让模型回答一句话”，而是把客服 Agent 做成可以持续演进的服务：有稳定 API、有幂等重试、有会话并发控制、有显式状态、有工具边界、有安全规则、有记忆、有成本控制、有降级、有观测、有质量评估、有 DeepSeek 主路径、有 FAQ/RAG、有 RocketMQ outbox、有管理接口、有部署编排、有压测入口和 100 题自动评测基线。
