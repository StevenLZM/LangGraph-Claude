...
**主要交付**
- Dockerfile
- Docker Compose：`api`、`redis`、`postgres/pgvector`、`prometheus`、`grafana`；Compose 默认使用 Postgres 业务存储和 Postgres checkpointer
- Locust 压测脚本
...
### 4. 业务存储和 LangGraph checkpoint 分层

`SessionStore`/`UserMemoryManager` 保存客服业务可见数据：消息窗口、质量评估 metadata、转人工原因、用户偏好和用户可删除记忆。LangGraph checkpointer 保存图执行状态，用于原生 checkpoint/resume。两者都可接 Postgres，但职责不同，不能用 checkpoint 表替代 `/sessions`、管理接口和记忆删除接口。
...
返回 API、Redis、数据库、业务存储、checkpointer、LLM 和限流后端配置的基础健康状态。
...
**实际交付**
- 新增 `memory/factory.py`：根据 `STORAGE_BACKEND=sqlite|postgres` 创建会话存储和用户记忆管理器。
- 扩展 `memory/session_store.py`：保留 SQLite `SessionStore`，新增 `PostgresSessionStore`，接口保持 `save_session/load_session/get_public_session/list_public_sessions/summarize_transfers` 不变。
- 扩展 `memory/long_term.py`：保留 SQLite `UserMemoryManager`，新增 `PostgresUserMemoryManager`，接口保持 `load_memories/save_from_turn/delete_memories/list_memories` 不变。
- 新增 `agent/checkpointing.py`：支持 `CHECKPOINTER_BACKEND=none|postgres|redis`，Postgres 默认复用 `DATABASE_URL`，Redis 默认复用 `REDIS_URL`，可用 `CHECKPOINTER_URL` 覆盖。
- `/health` 增加 `memory` 和 `checkpointer` 后端状态，`api/main.py` 使用工厂装配业务存储和 LangGraph checkpointer。
- Docker Compose 默认 `STORAGE_BACKEND=postgres`、`CHECKPOINTER_BACKEND=postgres`，本地 `.env.example` 默认仍是 `sqlite/none`，保证 pytest 和离线演示不依赖外部服务。
- `requirements.txt` 增加 `psycopg[binary,pool]`、`langgraph-checkpoint-postgres`、`langgraph-checkpoint-redis`。
- 新增 `tests/test_storage_backends.py` 和 `tests/test_checkpointer_factory.py`，覆盖后端选择、Postgres 存储契约、checkpointer URL/setup 行为。

**验收结果**
- `cd 05_PRODUCT_AGENT && pytest tests -q`：70 passed。
- `cd 05_PRODUCT_AGENT && docker compose config`：Compose 配置可展开，API 服务环境中包含 `STORAGE_BACKEND=postgres` 和 `CHECKPOINTER_BACKEND=postgres`。
