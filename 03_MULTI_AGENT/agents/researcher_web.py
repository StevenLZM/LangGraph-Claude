"""Web Researcher —— 走 registry.get_chain('web')；Tavily 主 / 外部 MCP Brave 兜底。"""
...
@safe_node
async def web_researcher_node(payload: dict) -> dict[str, Any]:
    from app.bootstrap import app_state  # 延迟 import 规避循环
    ...
    return {
        "evidence": evidence,
        "messages": [AIMessage(content=f"[web:{sq_id}] 收集 {len(evidence)} 条")],
    }
