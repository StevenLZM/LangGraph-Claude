...
- **T02 计算器**：已迁移为项目级 Codex Skill（`.codex/skills/calculator/`），不再作为运行时 tool 暴露
...
| "1234 * 5678 等于多少？" | 在 Codex 工作流中使用项目级 calculator skill；运行时 Agent 不再暴露计算器 tool |
| "写一个冒泡排序并运行测试" | 调用代码执行工具 |
| "特斯拉最新股价是多少？" | 调用搜索工具获取实时数据 |
| "今天是星期几？距离春节还有多少天？" | 运行时调用日期工具；精确日期差计算由项目级 calculator skill 辅助验证 |

---

## 八、评估标准

- [ ] 内置工具和 `.mcp.json` 动态加载的 MCP 工具均可正常调用并返回正确结果
